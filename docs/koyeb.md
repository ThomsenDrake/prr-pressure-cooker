...
The same worker registers the bounded `prr-case-event-step` workflow, the
legacy `case-lifecycle-workflow` compatibility workflow, and the Le Chat-facing
`prr-review-assistant` conversational workflow. Publish
...
the case store directly when no current lifecycle execution exists. It only
records the review decision; all outbound email, portal posting, payment, media,
legal notice sending, and case closure remain manual.
